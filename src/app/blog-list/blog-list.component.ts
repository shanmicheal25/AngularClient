...
export class BlogListComponent implements OnInit {
  ...
  public async sumbit(item, post_id) {
    ...
    this.commentData.post_id = post_id
    this.commentData.comment_text = this.textComment
    ...
    if(this.respData && this.respData.status == 200){
      ...
      item.comments.push(this.respData.commentsData)
      this.textComment = '';
    }
  ...
  public async sumbitPost() {
    ...
    debugger;
    if(this.respData && this.respData.status == 200){
      if(typeof(this.data.BlogsData) == 'undefined'){
        this.data.BlogsData.records = []
      }
      this.data.BlogsData.records.push(this.respData.BlogsData)
